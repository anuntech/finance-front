...
type InputProps = React.ComponentProps<"input"> & {
	isWithIcon?: boolean;
	isWithCommand?: boolean;
};

const Input = React.forwardRef<HTMLInputElement, InputProps>(
	(
		{ className, type, isWithIcon = false, isWithCommand = false, ...props },
		ref
	) => {
		return (
			<input
				...
				className={cn(
					...
					isWithIcon && "pl-10",
					isWithCommand &&
						"border-none text-foreground outline-none focus-visible:ring-0 focus-visible:ring-offset-0"
				)}
				ref={ref}
				{...props}
			/>
